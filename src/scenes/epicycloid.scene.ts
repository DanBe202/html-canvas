...
export class EpicycloidScene extends BaseScene {
  ...
  constructor(root: string, innerInitialDiameter: number, outerInitialDiameter: number, initialPointDistance: number) {
    ...
    this._innerCircle = new CircleObject(this.canvas, this.canvas.width / 2, this.canvas.height / 2, this._innerDiameter / 2, Colors.PrussianBlue);
    this._outerCircle = new CircleObject(this.canvas, this._innerCircle.x, this._innerCircle.y - this._innerDiameter / 2, this._outerDiameter / 2, Colors.PrussianBlue);
